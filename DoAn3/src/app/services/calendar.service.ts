...
export class CalendarService {
  ...
  getCalendarLunar(dd: number, mm: number, yy: number): string {
    let lunar = new Array();
    lunar = this.computeCalendarLunar(dd, mm, yy);
    let app: string = '';
    app = lunar[0] + '/' + lunar[1] + '/' + lunar[2] + '/' + lunar[3];
    return app;
  }
  computeCalendarLunar(dd: number, mm: number, yy: number): Array<string> {
  ...
  INT(d: number) {
   ...
   getLunarMonth11(yy: number, timeZone: number) {
    const PI = Math.PI;
    // tslint:disable-next-line:one-variable-per-declaration
   ...
   getLeapMonthOffset(a11: number, timeZone: number) {
    const PI = Math.PI;
    // tslint:disable-next-line:one-variable-per-declaration
   ...
   convertLunar2Solar(lunarDay: number, lunarMonth: number, lunarYear: number, lunarLeap: number, timeZone: number) {
    const PI = Math.PI;
    // tslint:disable-next-line:one-variable-per-declaration
